...
export default function DigestiveTimeline({ food }: { food: FoodItem | null }) {
  return (
    <div className="relative max-w-5xl mx-auto py-10">
      ...
      <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-0.5 bg-slate-800 -translate-x-1/2 overflow-hidden">
        <motion.div 
           className="w-full h-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)] origin-top"
           initial={{ scaleY: 0 }}
              ...
              <div className={`w-full pl-16 md:pl-0 md:w-1/2 ${isEven ? 'md:pl-12' : 'md:pr-12'} z-10`}>
                <div className={`p-6 rounded-2xl border ${organ.color.split(' ')[1]} bg-[#0d1117]/80 hover:bg-[#161b22] transition-all group`}>
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    <div className={`px-2 py-1 rounded border ${organ.color.split(' ')[1]} ${organ.color.split(' ')[0]} ${organ.color.split(' ')[2]} text-[10px] font-mono uppercase`}>
                      pH: {organ.ph}
                    </div>
                  </div>
                  
                  <h3 className="text-xl font-mono text-white mb-4 uppercase">{organ.name}</h3>
                  
                  <div className="space-y-5">
                    <div>
                      <span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono mb-1 block">Enzimas Atuantes</span>
                      <p className="font-mono text-blue-400 text-sm">{organ.enzymes.join(' • ')}</p>
                    </div>
                    
                    <div className="h-px w-full bg-slate-800" />

                    <div>
                      <span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono mb-1 block">Ação Fisiológica (Substrato → Produto)</span>
                      <p className="text-slate-300 text-sm">{organ.action}</p>
                    </div>

                    <div>
                      <span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono mb-1 block">Papel na Nutrição</span>
